...
### File map is a guide, not a complete contract

Each sprint's phase document includes an annotated file map listing every file with `CREATE` or `MODIFY`. The agent treats this map as the primary signal for what files are in scope for a sprint.

However, the file map may have omissions. A task step that explicitly names a file to modify is authoritative over a file map that silently omits it. The two tiers of resolution are:

**Tier 1 — Task step explicitly names the file.**
The agent modifies the file and reports it as a Category C auto-correction. No user confirmation is needed. Example: Task 5 Step 2 says "replace the stub callback in `OnlyOfficeController.java`" — that instruction overrides the file map's silence on that file.

**Tier 2 — Agent wants to touch a file not named anywhere in the phase doc.**
The agent stops and reports a Category A blocker. It does not proceed until the user confirms.

The test is: *"Does the phase document task body explicitly instruct this edit?"* If yes, proceed and report. If no, stop and ask.

#### Known file map gap — Sprint 1.4

**File:** `onlyoffice/api/OnlyOfficeController.java`
**Missing label:** `MODIFY`
**Source of authority:** Sprint 1.4 phase doc, Task 5, Step 2 explicitly instructs wiring `OnlyOfficeSaveHandler` into this controller.
**Resolution:** The agent modifies `OnlyOfficeController.java` as instructed and reports:

```
⚠️  AUTO-CORRECTION APPLIED — Sprint 1.4, Task 5
File:   fos-workspace-service/src/main/java/com/fos/workspace/onlyoffice/api/OnlyOfficeController.java
Change: File was missing MODIFY label in sprint 1.4 file map.
        Modified as explicitly instructed by Task 5, Step 2 (wire OnlyOfficeSaveHandler callback).
Review this file before committing Task 5.
```
...
Check prerequisite gate
    ...
    └── Gate passes
            ...
            │
            ├── File not in map AND not named in any task step
            │       → STOP, report blocker, wait for user
            │
...
| On file missing from map but named in a task step | Auto-correct, modify file, inform user, continue |
| On file missing from map AND not named in any task step | Stop, report blocker, wait |
