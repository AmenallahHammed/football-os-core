...
class PlayerProfileIntegrationTest extends FosTestContainersBase {
    ...
    static {
        ...
        try {
            ...
            System.err.println("[PlayerProfileIntegrationTest] Mongo container unavailable, falling back to external MongoDB: " + ex.getMessage());
        }
    ...
    void setUp() {
        ...
        wireMock().stubFor(get(urlMatching("/api/v1/players/.*"))
                .willReturn(okJson("""
                        {"id":"%s","name":"Test Player","position":"ST","nationality":"SA","dateOfBirth":"1998-04-15","currentTeamId":null}
    ...
    void should_return_profile_with_documents_section() {
        ...
        ResponseEntity<PlayerProfileResponse> response = restTemplate.getForEntity(
                "/api/v1/profiles/players/" + TEST_PLAYER_ID,
    ...
    @Test
    void should_return_profile_with_medical_section() {
        InitiateUploadRequest initiateRequest = new InitiateUploadRequest(
                "Medical Clearance",
        ...
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(response.getBody()).isNotNull();
        assertThat(response.getBody().medicalRecords()).isNotNull();
